...
namespace Game.StatsCalculators
{
    public class BasicPlayerWeaponStatsCalculator : IWeaponStatsCalculator
    {
        ...
        {
            float totalBaseDamageFromUpgrades = upgrades.Where(upgrade => upgrade.Type == WeaponUpgradeType.BaseDamage).ToList().Sum(upgrade => upgrade.Value);
            float totalDamageMultiplierFromUpgrades = Mathf.Max(1f,upgrades.Where(upgrade => upgrade.Type == WeaponUpgradeType.DamageMultiplier).ToList().Sum(upgrade => upgrade.Value));
        ...
        {
            float totalCooldownReductionFromUpgrades = upgrades.Where(upgrade => upgrade.Type == WeaponUpgradeType.CooldownReduction).Aggregate(1f, (current, upgrade) => current * ((100 - upgrade.Value) * 0.01f));
        ...
        {
            float totalProjectileSpeedFromUpgrades = Mathf.Max(1f, upgrades.Where(upgrade => upgrade.Type == WeaponUpgradeType.ProjectileSpeed).ToList().Sum(upgrade => upgrade.Value));
        ...
        {
            float totalProjectileSizeFromUpgrades = Mathf.Max(1f, upgrades.Where(upgrade => upgrade.Type == WeaponUpgradeType.ProjectileSize).ToList().Sum(upgrade => upgrade.Value));
